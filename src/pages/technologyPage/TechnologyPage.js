...
export const TechnologyPage = () => {
  ...
  const getTechnologyData = async (id) => {
    try {
      const resp = await fetch(
        `https://portfolio-backend-qaq3.onrender.com/api/technology?technology=${id}`
      )
  ...
  const getProyectsData = async (id) => {
    try {
      const resp = await fetch(
        `https://portfolio-backend-qaq3.onrender.com/api/proyect?searchBySkill=${id}`
      )
      const { data } = await resp.json()
      setProyects(data)
    } catch (error) {
  ...
  useEffect(() => {
    if (params.id) {
      getTechnologyData(params.id)
      getProyectsData(params.id)
    }
  ...
  return (
    <MainContainer>
      <BackContainer>
        <i onClick={() => history.push('/')} className='ri-arrow-left-line'></i>
      </BackContainer>
      {technology && (
        <Header>
